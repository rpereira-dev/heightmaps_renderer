...
void worldInit(t_world * world, char * bmpfile, float max_height, long seed) {
	...
	world->generators = array_list_new(16, sizeof(generator));
	generatorsInit(world);
...
void worldDelete(t_world * world) {
	...
	generatorsDelete(world);
}

t_generator * worldGetGeneratorAt(t_world * world, float wx, float wz) {
	return (array_list_get(world->generators, 0));
}
	...
	for (gridX = indexx ; gridX  < maxx; gridX++) {
		for (gridY = indexy ; gridY < maxy; gridY++) {
			...
			if (worldGetTerrain(world, gridX, gridY) == NULL) {
				...
				t_generator * generator = worldGetGeneratorAt(world, wx, wz);
				if (!generator->canGenerateAt(world, generator, wx, wz)) {
					continue ;
				}
